...
medicosRouter.post('/', authorize('SEC_ADM', 'REGULACAO'), async (req: Request, res: Response) => {
    try {
        ...
        await logAction({ req, acao: 'CREATE', entidade: 'MEDICO', entidadeId: medico.id, detalhes: `Médico ${medico.nome} cadastrado na unidade ${medico.unidadeId}.` });
...
medicosRouter.put('/:id', authorize('SEC_ADM', 'REGULACAO'), async (req: Request, res: Response) => {
    try {
        ...
        await logAction({ req, acao: 'UPDATE', entidade: 'MEDICO', entidadeId: medico.id, detalhes: `Médico ${medico.nome} atualizado.` });
        res.json(medico);
    } catch (err) {
        if (err instanceof z.ZodError) return res.status(400).json({ error: 'Erro de validação.' });
        throw err;
...
medicosRouter.delete('/:id', authorize('SEC_ADM'), async (req: Request, res: Response) => {
    ...
    await logAction({ req, acao: 'UPDATE', entidade: 'MEDICO', entidadeId: medico.id, detalhes: `Médico ${medico.nome} desativado.` });
